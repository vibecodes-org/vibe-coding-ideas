...
/**
 * Editable empty-state content — designed to be rendered inside a Dialog
 * over the placeholder columns. No outer flex/center wrapper; the Dialog
 * primitive handles centring.
 */
export function BoardEmptyStateContent({
  canUseAi,
  hasByokKey,
  starterCredits,
  onAiGenerate,
  ...
  return (
    <div className="text-center">
      ...
      <button
        ...
        onClick={onDismiss}
      >
        Skip &mdash; I&apos;ll add tasks manually
      </button>
      {onImport && (
        <button
          type="button"
          className="mt-1.5 block mx-auto text-xs text-muted-foreground hover:text-foreground hover:underline transition-colors"
          onClick={onImport}
        >
...
/**
 * Read-only viewers see this simpler centred message — no CTAs, no nudges.
 */
export function BoardEmptyStateReadOnly() {
  return (
