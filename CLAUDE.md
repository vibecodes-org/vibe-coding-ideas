...
- **Template Edit Propagation**: When a template's steps are updated, `propagateTemplateEdits()` in `src/lib/workflow-helpers.ts` automatically pushes changes to `pending` steps in active workflow runs. Skips runs with step count mismatch (structural changes). Does NOT re-match `bot_id`. Called from both `updateWorkflowTemplate` server action and MCP tool
