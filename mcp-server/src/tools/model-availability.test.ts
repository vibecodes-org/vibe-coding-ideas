...
const NOW = new Date("2026-09-25T09:15:00.000Z");

describe("isMarkerLive", () => {
  it("is live for a marker set earlier the same UTC day", () => {
    expect(isMarkerLive("2026-09-25T00:00:01.000Z", NOW)).toBe(true);
    expect(isMarkerLive("2026-09-25T09:14:59.000Z", NOW)).toBe(true);
  });

  it("expires at the next UTC midnight (AC-5)", () => {
    expect(isMarkerLive("2026-09-24T23:59:59.000Z", NOW)).toBe(false);
  });
  ...
  it("never honours a future marker (clock skew must not strand a model)", () => {
    expect(isMarkerLive("2026-09-25T23:00:00.000Z", NOW)).toBe(false);
  });

  it("accepts a Date as well as a string", () => {
    expect(isMarkerLive(new Date("2026-09-25T05:00:00.000Z"), NOW)).toBe(true);
  });

  it("agrees with markerWindowStart at the exact boundary", () => {
    const start = markerWindowStart(NOW);
    expect(start).toBe("2026-09-25T00:00:00.000Z");
...
describe("applyModelAvailability", () => {
  ...
  it("leaves the resolution untouched with undefined too", () => {
    expect(applyModelAvailability(frontier, undefined).switched).toBe(false);
  });
  ...
  it("names the old primary as the new fallback (a second try, not a dead end)", () => {
    expect(applyModelAvailability(frontier, "fable").fallback).toBe("fable");
  });
});
...
describe("modelSwitchNotice", () => {
  it("names the dead model, the backup, and the reason (AC-3)", () => {
    ...
    expect(notice).toMatch(/midnight UTC/);
    expect(notice).toMatch(/^MODEL SWITCH:/);
  });

  it("says the board, not the account, is scoped", () => {
    const notice = modelSwitchNotice(applyModelAvailability({ resolved: "fable", fallback: "opus" }, "fable"));
    expect(notice).toMatch(/on this board/);
  });
...
describe("shouldRescueStep", () => {
  it("rescues a first model-unavailability failure (AC-1)", () => {
    ...
    expect(shouldRescueStep(true, undefined)).toEqual({ rescue: true, reason: "rescued" });
  });
  ...
  it("leaves ordinary failures alone", () => {
    ...
    expect(shouldRescueStep(false, false).reason).toBe("not-a-model-failure");
  });
});
...
describe("rescueSentence", () => {
  it("explains the switch, scopes it to steps that would have used the dead model, and says it happens only once (binding Design Review wording)", () => {
    ...
    expect(sentence).toContain(
      "Other steps on this board that would have used **fable** will use **opus** too until midnight UTC, when **fable** is tried again."
    );
    // The rejected wording ("every other step" / unscoped "will use") must not appear.
    expect(sentence).not.toMatch(/Other steps on this board will use/);
  });
});

describe("secondFailureSentence", () => {
  it("names the model this attempt was actually directed at and says no retry happens", () => {
    const sentence = secondFailureSentence("opus");
    expect(sentence).toContain("opus");
    expect(sentence).toMatch(/No automatic retry/);
    expect(sentence).toMatch(/left failed/);
  });
});
